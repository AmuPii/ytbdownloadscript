...
def baixar(url, output_dir, hook_progresso, qualidade):
    """
    Engine dedicada para Instagram (Reels e Vídeos).
    Versão Estável: Sem conversão de GIF.
    """
    
    # 1. Definição da Qualidade
    # No Instagram não há muita escolha de resolução, pegamos o melhor disponível
    if qualidade == "MP3":
    ...
    ydl_opts = {
        'format': format_str,
        # O ID costuma ser mais seguro que o título no Instagram
        'outtmpl': os.path.join(output_dir, 'Instagram_%(id)s.%(ext)s'),
        'progress_hooks': [utils.DownloadLogger(hook_progresso).hook],
        'quiet': True,
        'no_warnings': True,
        
        # Aponta para a pasta ./ffmpeg/bin/
