...
def baixar(url, output_dir, hook_progresso, qualidade):
    ...
    if qualidade == "MP3":
        ...
        # Facebook nem sempre expõe resoluções exatas, mas tentamos filtrar
    ...
    ydl_opts = {
        'format': format_str,
        'outtmpl': os.path.join(output_dir, 'Facebook_%(title)s [%(id)s].%(ext)s'),
